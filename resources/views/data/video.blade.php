...
<div class="bg-pink-100 min-h-screen py-10">    
    <div class="container mx-auto bg-white p-8 rounded-lg shadow-md max-w-2xl mt-20" style="margin-right: 260px;">
        <h1 class="text-center text-xl text-pink-600 font-bold mb-10 leading-tight tracking-tight md:text-2xl">{{ $user->username }}'s Video</h1>

        <form action="/home/inbox/video/{{(int)$aesuser->user_id}}" method="post" enctype="multipart/form-data" class="flex flex-col mb-6">
            @csrf
        ...
        <div class="hidden mb-6" id="hiddendata">
            <label class="font-bold text-sm text-pink-600 mb-2">Here is {{ $user->username }}'s Video</label>
            @php
            $ckey = null;
            if ($inbox !== null) {
            ...
            @if($ckey !== null)
            <a href="/download/aes/video/{{ $aesuser->user_id }}/{{ $ckey }}" class="bg-pink-600 hover:bg-pink-500 text-white px-3 py-1 rounded-md text-sm">Download</a>
            @endif
        </div>
        ...
        <form action="/verify/{{(int)$aesuser->user_id}}" method="post" enctype="multipart/form-data" class="flex flex-col space-y-4 mt-6">
            ...
            <div class="flex flex-col">
                <label class="font-bold text-sm text-pink-600 mb-2">Verify Video</label>
        ...
        <div class="flex flex-col mt-5 space-y-4">
            <div class="flex flex-col">
                <label class="font-bold text-sm text-pink-600 mb-2">Digest from Video</label>
            ...
            <div class="bg-green-100 text-green-700 p-4 rounded">Digest and decrypted digital signature match! Video is verified.</div>
            @else
            <div class="bg-red-100 text-red-700 p-4 rounded">Digest and decrypted digital signature do not match. Video integrity is compromised.</div>
            @endif
        </div>
